...
def infer_growth_rate(curve_v, ttu, alphas=None, eps_L=.0001, positive=False):
    ...
    eps_L
      Negligible factor for (inverse) derivation matrix
...
def infer_synthesis_rate_onestep(curve_f, curve_v, ttu, degr,
    ...
    if isinstance(curve_f, list):
        ...
        results = [infer_synthesis_rate_onestep(f, v, ttu, degr, alphas=alphas, eps_L=eps_L)
